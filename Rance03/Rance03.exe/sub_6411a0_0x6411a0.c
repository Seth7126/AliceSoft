            ...
            if (not(*(&var_3c + (ecx << 2)) f> xmm1_1))
                ...
                while (true)
                    if (esi_1 s>= eax_5[ecx - 1] + arg2 || ecx s<= 1
                            || *(var_10_2 + ebx_2) f< *(&var_3c + (ecx << 2))
                            || esi_1 s>= *ebx_2 + arg2)
                        ebx = var_c
                        break
                    
                    void var_40
                    int32_t xmm0_3 = *(&var_40 + (ecx << 2))
                    ecx -= 1
                    eax_5 = var_c
                    ebx_2 -= 4
...
if (ecx s> 0)
    result = &ebx[1]
    void* var_1c_2 = &__saved_edi - ebx
    int32_t* result_1 = result
    
    do
        int32_t ebx_5
        
        if (esi_2 s>= ecx - 1 || *(result + var_1c_2) f<= (&__saved_edi)[esi_2])
        ...
        if (edi s< ebx_5)
            if (ebx_5 - edi s>= 4)
                ...
                int32_t var_28_1 = ((ebx_5 - edi - 4) u>> 2) + 1
